...
// BFS function to find the goal state
...
void renderBoard(sf::RenderWindow& window, const vector<vector<char>>& board, int cellSize, const vector<sf::Vertex>& path) {
    ...
    for (int i = 0; i < board.size(); i++) {
        for (int j = 0; j < board[i].size(); j++) {
            ...
            } else if (i == board.size() - 1 && j == board[i].size() - 3) {
...
int main() {
    ...
    int rows = 7, cols = 9;
    int cellSize = 80; // Adjust cell size for better fit
    ...
    node* result = bfs(board, goalBoard);
